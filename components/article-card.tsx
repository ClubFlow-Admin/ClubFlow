...
/** Appends ?from=<key> so the article page can show a "Back to X" link to where the reader actually came from. */
function articleHref(slug: string, from?: string) {
  return from ? `/articles/${slug}?from=${from}` : `/articles/${slug}`;
}
...
export function FeaturedArticleCard({ article, priority = false, image, from }: { article: ArticleWithRelations; priority?: boolean; image?: ResolvedImage; from?: string }) {
  const location = formatLocation(article.city, article.state);
  const resolved = image ?? imageForArticle(article, 1400, 1000);
  const href = articleHref(article.slug, from);
  return (
    <article className="card-lift group grid overflow-hidden border bg-white lg:grid-cols-[1.08fr_.92fr]">
      <Link href={href} className="relative block min-h-[280px] overflow-hidden no-underline sm:min-h-[400px]">
      ...
      <div className="flex flex-col justify-center p-6 sm:p-9">
        ...
        <Link href={href} className="mt-6 inline-flex items-center gap-2 border-t pt-5 text-sm font-black text-primary no-underline transition group-hover:gap-3">Read intelligence brief <ArrowRight className="h-4 w-4" /></Link>
      </div>
    </article>
  );
}

export function CompactArticleRow({ article, showCategory = true, inverse = false, from }: { article: ArticleWithRelations; showCategory?: boolean; inverse?: boolean; from?: string }) {
  const href = articleHref(article.slug, from);
  return (
    <article className={`group grid gap-2 border-b py-4 last:border-0 sm:grid-cols-[1fr_auto] sm:items-start ${inverse ? "border-white/15" : ""}`}>
      <div>
        ...
        <Link href={href} className="no-underline"><h3 className={`mt-1.5 font-serif text-lg font-black leading-snug transition ${inverse ? "text-white group-hover:text-emerald-300" : "group-hover:text-primary"}`}>{article.title}</h3></Link>
      </div>
      <Link href={href} aria-label={`Read ${article.title}`} className={`mt-1 hidden no-underline transition sm:block ${inverse ? "text-emerald-300 group-hover:translate-x-0.5" : "text-primary group-hover:translate-x-0.5"}`}><ArrowRight className="h-4 w-4" /></Link>
    </article>
  );
}

export function SectionArticleCard({ article, image, from }: { article: ArticleWithRelations; image?: ResolvedImage; from?: string }) {
  const location = formatLocation(article.city, article.state);
  ...
  return (
    <article className="card-lift group flex h-full flex-col overflow-hidden border bg-white">
      <Link href={href} className="relative block h-44 overflow-hidden no-underline">
      ...
      <div className="flex flex-1 flex-col p-5 sm:p-6">
        ...
        <Link href={href} className="no-underline"><h2 className="font-serif mt-2.5 text-2xl font-black leading-tight transition group-hover:text-primary">{article.title}</h2></Link>
        ...
        <Link href={href} className="mt-auto inline-flex items-center gap-2 pt-5 text-xs font-black uppercase tracking-[.08em] text-primary no-underline transition group-hover:gap-3">Read brief <ArrowRight className="h-3.5 w-3.5" /></Link>
      </div>
    </article>
  );
}
