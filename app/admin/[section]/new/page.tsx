...
export default async function NewSectionContentPage({ params }: { params: Promise<{ section: string }> }) {
  ...
  if(section.kind==="article") { const [category,sources,mediaAssets,entityOptions]=await Promise.all([prisma.category.findUnique({where:{slug:section.categorySlug}}),getSources(),prisma.mediaAsset.findMany({orderBy:{title:"asc"}}),getEntityTaggingOptions()]); if(!category) notFound(); return <Shell label={section.label} mode="Add New"><AdminArticleForm action={action} lockedCategory={category} sources={sources} mediaAssets={mediaAssets} entityOptions={entityOptions} /></Shell>; }
