...
export default async function EditSectionContentPage({params}:{params:Promise<{section:string;id:string}>}) {
  ...
  if(section.kind==="article") { const [article,category,sources,mediaAssets]=await Promise.all([prisma.article.findFirst({where:{id,category:{slug:section.categorySlug}},include:{category:true,source:true,heroImage:true}}),prisma.category.findUnique({where:{slug:section.categorySlug}}),getSources(),prisma.mediaAsset.findMany({orderBy:{title:"asc"}})]); if(!article||!category) notFound(); return <Shell label={section.label}><AdminArticleForm action={action} article={article} lockedCategory={category} sources={sources} mediaAssets={mediaAssets} /></Shell>; }
