...
import { updateSectionContent, updateSectionContentAndContinue } from "@/app/admin/section-actions";
...
export default async function EditSectionContentPage({params}:{params:Promise<{section:string;id:string}>}) {
  ...
  if(section.kind==="article") { const [article,category,sources,mediaAssets,entityOptions]=await Promise.all([prisma.article.findFirst({where:{id,category:{slug:section.categorySlug}},include:{category:true,source:true,heroImage:true,clubs:true,companies:true,people:true}}),prisma.category.findUnique({where:{slug:section.categorySlug}}),getSources(),prisma.mediaAsset.findMany({orderBy:{title:"asc"}}),getEntityTaggingOptions()]); if(!article||!category) notFound(); const continueAction=updateSectionContentAndContinue.bind(null,section.slug,id); return <Shell label={section.label}><AdminArticleForm action={action} continueAction={continueAction} article={article} lockedCategory={category} sources={sources} mediaAssets={mediaAssets} entityOptions={entityOptions} openAiConfigured={isOpenAIConfigured()} /></Shell>; }
