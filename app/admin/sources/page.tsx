...
import { AlertTriangle, CheckCircle2, Edit, ExternalLink, History, PauseCircle, PlayCircle, Plus, Rss, Search, Trash2 } from "lucide-react";
import type { Prisma } from "@prisma/client";
import { deleteSource, setSourceActive } from "@/app/admin/sources/actions";
import { AdminIngestButton } from "@/components/admin-ingest-button";
import { AdminNewsroomOpsPanel } from "@/components/admin-newsroom-ops-panel";
import { AdminSourceRefreshButton } from "@/components/admin-source-refresh-button";
import { AdminTabs } from "@/components/admin-tabs";
import { Badge } from "@/components/ui/badge";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { getNewsroomOpsSummary } from "@/lib/newsroom-ops";
import { isOpenAIConfigured } from "@/lib/openai";
import { prisma } from "@/lib/prisma";
import { feedAvailabilityLabel, sourceCategories, sourceCategoryLabel, sourcePriorityLabel, sourceTypeLabel, sourceTypes } from "@/lib/source-options";
...
export default async function SourcesPage({ searchParams }: { searchParams: Promise<{ q?: string; status?: string; type?: string; category?: string }> }) {
  ...
  return <main className="container-shell py-8"><AdminTabs />
    ...
    <AdminNewsroomOpsPanel summary={opsSummary} />
    ...
    <div className="overflow-x-auto rounded-lg border bg-white"><table className="w-full min-w-[1100px] text-left text-sm"><thead className="border-b bg-muted text-xs uppercase text-muted-foreground"><tr><th className="px-4 py-3">Source</th><th className="px-4 py-3">Coverage</th><th className="px-4 py-3">Priority</th><th className="px-4 py-3">Status</th><th className="px-4 py-3">Monitoring</th><th className="px-4 py-3">Articles</th><th className="px-4 py-3 text-right">Actions</th></tr></thead><tbody>
      {sources.map((source) => { const health = computeSourceHealth(source, statsFor(statsMap, source.id)); return <tr key={source.id} className="border-b align-top last:border-0"><td className="px-4 py-4"><div className="font-black">{source.name}</div>{source.organization?<div className="text-xs text-muted-foreground">{source.organization}</div>:null}<div className="mt-1 flex gap-3 text-xs">{source.homepageUrl ? <a href={source.homepageUrl} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 text-primary">Website <ExternalLink className="h-3 w-3" /></a> : <span className="text-muted-foreground">No website</span>}{source.rssUrl ? <a href={source.rssUrl} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 text-primary">RSS <Rss className="h-3 w-3" /></a> : <span className="text-muted-foreground">No RSS</span>}</div><Badge className={source.feedAvailability==="available"?"mt-2 border-emerald-200 bg-emerald-50 text-emerald-800":"mt-2 border-amber-200 bg-amber-50 text-amber-800"}>{feedAvailabilityLabel(source.feedAvailability)}</Badge>{source.notes ? <p className="mt-2 max-w-xs text-xs leading-5 text-muted-foreground">{source.notes}</p> : null}</td><td className="px-4 py-4"><div className="font-semibold">{sourceTypeLabel(source.sourceType)}</div><div className="mt-1 text-xs text-muted-foreground">Primary: {sourceCategoryLabel(source.primaryCategory)}</div><div className="mt-2 flex max-w-xs flex-wrap gap-1">{source.categories.map((item)=><Badge key={item} className="border-slate-200 bg-white text-slate-700">{sourceCategoryLabel(item)}</Badge>)}</div></td><td className="px-4 py-4"><Badge className={source.priority>=75?"border-amber-200 bg-amber-50 text-amber-800":"border-slate-200 bg-slate-50 text-slate-700"}>{sourcePriorityLabel(source.priority)}</Badge></td><td className="px-4 py-4"><div className="flex flex-col gap-1.5">{source.active?<Badge className="border-emerald-200 bg-emerald-50 text-emerald-800"><CheckCircle2 className="mr-1 h-3 w-3" />Active</Badge>:<Badge className="border-slate-200 bg-white text-slate-700">Inactive</Badge>}<Link href="/admin/sources/health" className="no-underline"><Badge className={healthBadgeClass(health)}>{sourceHealthLabels[health]}</Badge></Link></div></td><td className="px-4 py-4 text-xs leading-5 text-muted-foreground"><div>Checked: {dateLabel(source.lastCheckedAt)}</div><div>Imported: {dateLabel(source.lastSuccessfulImportAt)}</div><div>Every {source.importFrequencyMinutes}m</div></td><td className="px-4 py-4 font-black">{source._count.articles}</td><td className="px-4 py-4"><div className="flex flex-col items-end gap-2"><div className="flex gap-2"><Button asChild size="sm" variant="outline"><Link href={`/admin/sources/${source.id}/edit`} className="no-underline"><Edit className="h-4 w-4" /> Edit</Link></Button><form action={setSourceActive.bind(null,source.id,!source.active)}><Button type="submit" size="sm" variant="ghost">{source.active?<PauseCircle className="h-4 w-4" />:<PlayCircle className="h-4 w-4" />}{source.active?"Deactivate":"Activate"}</Button></form></div><div className="flex gap-2"><Button asChild size="sm" variant="ghost"><Link href={`/admin/sources/${source.id}/history`} className="no-underline"><History className="h-4 w-4" /> History</Link></Button><AdminSourceRefreshButton sourceId={source.id} /></div><form action={deleteSource.bind(null,source.id)}><Button type="submit" size="sm" variant="ghost" className="text-red-600 hover:text-red-700"><Trash2 className="h-4 w-4" /> Delete</Button></form></div></td></tr>; })}
