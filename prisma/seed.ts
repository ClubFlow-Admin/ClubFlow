...
async function main() {
  ...
  for (const [title, clubName, city, state, description] of jobs) await prisma.jobPosting.upsert({ where: { title_clubName: { title, clubName } }, update: { city, state, description, url: `https://example.com/jobs/${slugify(`${clubName}-${title}`)}` }, create: { title, clubName, city, state, description, url: `https://example.com/jobs/${slugify(`${clubName}-${title}`)}` } });
  for (const [executive, newRole, previousRole, clubName, city, state] of moves) { const existing = await prisma.executiveMove.findFirst({ where: { executive, clubName } }); if (existing) await prisma.executiveMove.update({ where: { id: existing.id }, data: { newRole, previousRole, city, state, effectiveAt: new Date("2026-06-16"), notes: "Demo-safe leadership update prepared for the ClubFlow preview." } }); else await prisma.executiveMove.create({ data: { executive, newRole, previousRole, clubName, city, state, effectiveAt: new Date("2026-06-16"), notes: "Demo-safe leadership update prepared for the ClubFlow preview." } }); }
  ...
  for (const [category, rank, clubName, city, state, score, rationale] of rankings) await prisma.rankingEntry.upsert({ where: { category_clubName: { category, clubName } }, update: { rank, city, state, score, rationale }, create: { category, rank, clubName, city, state, score, rationale } });
  for (const [showName, title, description, duration] of podcasts) await prisma.podcastEpisode.upsert({ where: { title }, update: { showName, description, duration, comingSoon: true }, create: { showName, title, description, duration, comingSoon: true } });
